...
class CategorieController extends Controller
{
    ...
    public function deleteForm(string $id): void
    {
        ...
        if (Categorie::countEquipements((int) $id) > 0) {
            $this->flash('error', 'Impossible de supprimer : des équipements appartiennent à cette catégorie.');
            Router::redirect('categories');
        }

        $this->view('confirm/index', [
            'confirmTitle'   => 'Supprimer la catégorie « ' . $categorie['nom'] . ' »',
            'confirmMessage' => 'Vous êtes sur le point de supprimer cette catégorie.',
            'confirmMessage2' => 'Cette action est définitive et irréversible.',
            'confirmIcon'    => 'fa-trash',
            'confirmPost'    => BASE_URL . 'categories/supprimer/' . $id,
            'confirmButton'  => 'Supprimer la catégorie',
            'confirmButtonClass' => 'btn-danger-ghost',
            'confirmButtonIcon'  => 'fa-trash',
            'backUrl'        => BASE_URL . 'categories',
            'backLabel'      => 'Retour à la liste',
            'confirmRows'    => [
                ['label' => 'Nom',        'value' => $categorie['nom']],
                ['label' => 'Description','value' => $categorie['description'] ?: '—'],
            ],
        ]);
    }
