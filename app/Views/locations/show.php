...
<div class="panel workflow reveal">
    <div class="panel-head"><h3><i class="fas fa-bolt"></i> Actions de traitement</h3></div>
    <div class="workflow-actions">
        <?php if ($location['statut'] === 'en_attente'): ?>
            <form method="post" action="<?= BASE_URL ?>location/<?= (int) $location['id'] ?>/confirmer" data-confirm="Confirmer cette location ? Le stock sera réservé.">
                <?= \App\Core\Csrf::field() ?>
                <button type="submit" class="btn btn-success"><i class="fas fa-check"></i> Confirmer la demande</button>
            </form>
            <form method="post" action="<?= BASE_URL ?>location/<?= (int) $location['id'] ?>/refuser" data-confirm="Refuser cette demande ?">
                <?= \App\Core\Csrf::field() ?>
                <button type="submit" class="btn btn-danger-ghost"><i class="fas fa-xmark"></i> Refuser</button>
            </form>
        <?php elseif ($location['statut'] === 'confirmee'): ?>
            <form method="post" action="<?= BASE_URL ?>location/<?= (int) $location['id'] ?>/demarrer" data-confirm="Démarrer la location ?">
                <?= \App\Core\Csrf::field() ?>
            ...
            <form method="post" action="<?= BASE_URL ?>location/<?= (int) $location['id'] ?>/annuler" data-confirm="Annuler cette location ? Le stock sera libéré.">
                <?= \App\Core\Csrf::field() ?>
                <button type="submit" class="btn btn-danger-ghost"><i class="fas fa-ban"></i> Annuler</button>
            </form>
