        ...
        <form method="post" action="<?= BASE_URL ?>demande/<?= (int) $equipement['id'] ?>" class="form" data-location-form novalidate>
            ...
            <div class="form-grid-2">
                <div class="field">
                    ...
                    <div class="input-wrap"><i class="fas fa-calendar-day"></i>
                        <input type="date" id="date_debut" name="date_debut" min="<?= date('Y-m-d') ?>"
                               value="<?= e($old['date_debut'] ?? date('Y-m-d', strtotime('+1 day'))) ?>"
                               data-validate="required|date" data-date-debut>
                    </div>
                ...
                <div class="field">
                    ...
                    <div class="input-wrap"><i class="fas fa-calendar-day"></i>
                        <input type="date" id="date_fin" name="date_fin" min="<?= date('Y-m-d', strtotime('+1 day')) ?>"
                               value="<?= e($old['date_fin'] ?? date('Y-m-d', strtotime('+3 days'))) ?>"
                               data-validate="required|date|after:date_debut" data-date-fin>
                    </div>
                    <span class="field-error"></span>
                </div>
            </div>
            ...
            <div class="field">
                ...
                <div class="input-wrap"><i class="fas fa-boxes-stacked"></i>
                    <input type="number" id="quantite" name="quantite" min="1" max="<?= (int) $equipement['stock_disponible'] ?>"
                           value="<?= e($old['quantite'] ?? 1) ?>"
                           data-validate="required|int|min:1|max:<?= (int) $equipement['stock_disponible'] ?>" data-quantite>
                </div>
                <span class="field-error"></span>
            </div>
            ...
            <div class="field">
                <label class="checkbox">
                    <input type="checkbox" id="cgv" data-validate="required">
                    <span></span>
