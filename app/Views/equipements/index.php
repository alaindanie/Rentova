...
<div class="toolbar">
    <form method="get" action="<?= BASE_URL ?>equipements" class="toolbar-form" data-filter-form>
        ...
        <input type="number" name="prix_min" min="0" value="<?= e($filters['prix_min']) ?>" placeholder="Prix min">
        <input type="number" name="prix_max" min="0" value="<?= e($filters['prix_max']) ?>" placeholder="Prix max">
        ...
        <select name="etat">
            ...
            <option value="indisponible" <?= $filters['etat'] === 'indisponible' ? 'selected' : '' ?>>Indisponible</option>
        </select>
        <select name="tri">
        ...
        <table class="table">
            ...
            <?php foreach ($equipements as $eq): ?>
                <tr class="<?= (int) $eq['stock_disponible'] <= (int) $eq['seuil_alerte'] ? 'row-alert' : '' ?>">
                    ...
                    <td><b><?= e(montant($eq['prix_jour'])) ?></b></td>
                    ...
                    <td>
                        <span class="badge badge-<?= $eq['etat'] === 'disponible' ? 'success' : ($eq['etat'] === 'maintenance' ? 'warning' : 'danger') ?>">
                            <?= ucfirst(e($eq['etat'])) ?>
                        </span>
                    </td>
                    <td>
                        <div class="row-actions">
                            ...
                            <form method="post" action="<?= BASE_URL ?>equipements/supprimer/<?= (int) $eq['id'] ?>" class="inline" data-confirm="Supprimer cet équipement ?">
                                <?= \App\Core\Csrf::field() ?>
                                <button type="submit" class="btn-icon danger" title="Supprimer"><i class="fas fa-trash"></i></button>
                            </form>
                        </div>
                    </td>
                </tr>
